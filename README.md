...
A vibration-based predictive-maintenance prototype built for the Industrial IoT (IIoT) module at Hochschule Emden/Leer. A small desk fan stands in for industrial rotating equipment (CNC spindles, pumps, conveyor motors). An ESP32 + 6-axis accelerometer streams live vibration data through an MQTT → InfluxDB → Grafana pipeline, with an unsupervised machine-learning model (Isolation Forest) scoring machine health in real time — trained entirely on healthy data, with no labeled faults required.
 
**Team:** Rushikesh Jadhav & Venkatareddi Kumaar Elisetty
**Module:** Industrial IoT (IIoT), M.Eng. Industrial Informatics
**Supervisors:** Prof. Kutscher, Prof. Streekmann
...
| Test asset | Small USB-powered desk fan |
...
2. Fill in your WiFi SSID/password and your laptop's local IP address (`ipconfig` / `ifconfig`) for `mqtt_server`.
...
- WiFi network stability affects data continuity; a phone hotspot introduced periodic throttling during development (resolved via `WiFi.setSleep(false)`).
- Effective sample rate depends on correct InfluxDB point timestamping — points without explicit timestamps can collide and silently overwrite each other under batched writes (see `bridge.py` for the fix: `.time(time.time_ns(), WritePrecision.NS)`).
## Project structure
...
Academic project — Hochschule Emden/Leer, IIoT module.# Smart Machine Health Monitoring System
...
Academic project — Hochschule Emden/Leer, IIoT module.
